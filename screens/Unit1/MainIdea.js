...
const MainIdea = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      try {
        const mainIdeaQuestionsRef = collection(db, 'units', unit, 'MainIdea');
        const querySnapshot = await getDocs(mainIdeaQuestionsRef);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Main Idea - {unit}</Text>
      {quizCompleted ? (
